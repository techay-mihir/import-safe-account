...
const generateTarget = async (owners, threshold, guardAddress, logGasUsage) => {
    ...
    const safe = await (0, setup_1.getSafeWithOwners)(signers.map((owner) => owner.address), threshold, fallbackHandlerAddress, logGasUsage);
...
const setupBenchmarkContracts = (benchmarkFixture, logGasUsage) => {
    return hardhat_1.deployments.createFixture(async ({ deployments }) => {
        ...
        for (const config of exports.configs) {
            targets.push(await generateTarget(config.signers, config.threshold, config.useGuard ? guardAddress : constants_1.AddressZero, logGasUsage));
        }
        return {
            targets,
